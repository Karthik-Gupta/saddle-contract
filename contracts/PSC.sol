 ...
 */
contract PSC is ERC20Permit, Pausable, SimpleGovernance {
    ...
    /**
     * @notice Initializes PSC token with specified governance address and recipients. For vesting
    ...
    ) public ERC20("Pascal DAO", "PSC") ERC20Permit("Pascal DAO") {
        require(governance_ != address(0), "PSC: governance cannot be empty");
        require(
            vestingContractTarget_ != address(0),
            "PSC: vesting contract target cannot be empty"
        );
        require(
            pausePeriod_ > 0 && pausePeriod_ <= 52 weeks,
            "PSC: pausePeriod must be in between 0 and 52 weeks"
        );
     ...
     * release schedule can be found on <Pascal website>
        ...
        returns (address)
    {
        require(
            ...
            "PSC: duration for vesting cannot be 0"
        );
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused() || allowedTransferee[from], "PSC: paused");
        require(to != address(this), "PSC: invalid recipient");
    }
    ...
    function rescueTokens(
        ...
        require(to != address(0), "PSC: invalid recipient");
        ...
        if (token == IERC20(address(0))) {
            ...
            require(balance > 0, "PSC: trying to send 0 ETH");
            // slither-disable-next-line arbitrary-send
            (bool success, ) = to.call{value: balance}("");
            require(success, "PSC: ETH transfer failed");
        } else {
            // any other erc20
            ...
            balance = balance == 0
                ? totalBalance
                : Math.min(totalBalance, balance);
            require(balance > 0, "PSC: trying to send 0 balance");
