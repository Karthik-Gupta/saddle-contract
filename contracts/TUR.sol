 ...
 */
contract TUR is ERC20Permit, Pausable, SimpleGovernance {
    ...
    /**
     * @notice Initializes TUR token with specified governance address and recipients. For vesting
    ...
    ) public ERC20("Turing DAO", "TUR") ERC20Permit("Turing DAO") {
        require(governance_ != address(0), "TUR: governance cannot be empty");
        require(
            vestingContractTarget_ != address(0),
            "TUR: vesting contract target cannot be empty"
        );
        require(
            pausePeriod_ > 0 && pausePeriod_ <= 52 weeks,
            "TUR: pausePeriod must be in between 0 and 52 weeks"
        );
     ...
     * release schedule can be found on <Turing website>
        ...
        returns (address)
    {
        require(
            ...
            "TUR: duration for vesting cannot be 0"
        );
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused() || allowedTransferee[from], "TUR: paused");
        require(to != address(this), "TUR: invalid recipient");
    }
    ...
    function rescueTokens(
        ...
        require(to != address(0), "TUR: invalid recipient");
        ...
        if (token == IERC20(address(0))) {
            ...
            require(balance > 0, "TUR: trying to send 0 ETH");
            // slither-disable-next-line arbitrary-send
            (bool success, ) = to.call{value: balance}("");
            require(success, "TUR: ETH transfer failed");
        } else {
            // any other erc20
            ...
            balance = balance == 0
                ? totalBalance
                : Math.min(totalBalance, balance);
            require(balance > 0, "TUR: trying to send 0 balance");
